...
const SweatpantsScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const lastShuffleTime = await AsyncStorage.getItem('lastShuffleTime');
      const cachedProducts = await AsyncStorage.getItem('cachedSweatpantsProducts');
      ...
      if (!lastShuffleTime || now - parseInt(lastShuffleTime) > 1 * 60 * 1000 || !cachedProducts) {
        ...
        const response = await axios.get(`${BASE_URL}/products/`, {
          ...
          params: { clothing_category: 'sweatpants' }
        });
        ...
        await AsyncStorage.setItem('lastShuffleTime', now.toString());
        await AsyncStorage.setItem('cachedSweatpantsProducts', JSON.stringify(selectedProducts));
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('SweatpantsDetailScreen', { product, userId, initialIndex: selectedProfileIndex })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
