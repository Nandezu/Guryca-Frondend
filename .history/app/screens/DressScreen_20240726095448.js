...
const DressScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const lastFetchDate = await AsyncStorage.getItem('lastProductFetchDate');
      const storedProducts = await AsyncStorage.getItem('cachedDressProducts');
  
      if (lastFetchDate === today && storedProducts) {
        console.log("Using cached products for today");
        setProducts(JSON.parse(storedProducts));
        return;
      }
  
      console.log("Fetching and shuffling new products for today");
      ...
      console.log(`Selected ${selectedProducts.length} shuffled products for today`);
      setProducts(selectedProducts);
  
      // Uložení zamíchaných produktů a data do AsyncStorage
      await AsyncStorage.setItem('lastProductFetchDate', today);
