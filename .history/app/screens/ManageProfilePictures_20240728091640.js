...
const ManageProfileImage = () => {
  ...
  const loadProfileImages = async () => {
    try {
      ...
      if (response.data.profile_images && Array.isArray(response.data.profile_images)) {
        console.log('Loaded profile images:', response.data.profile_images);
  ...
  const uploadPhoto = async (uri) => {
    try {
      ...
      const response = await axios.post(`http://192.168.0.106:8000/user/users/${userId}/add_profile_image/?from_image_detail=true&is_new_upload=true`, formData, {
        headers: {
      ...
      if (response.status === 200 || response.status === 201) {
        console.log('Photo uploaded successfully. Response:', response.data);
        loadProfileImages();
      } else {
  ...
  const renderProfileItem = ({ item, index }) => {
    ...
    console.log('Rendering item:', item, 'isNewUpload:', isNewUpload);

    const imageStyle = isNewUpload
      ? {
          width: '115%',
          height: '100%',
