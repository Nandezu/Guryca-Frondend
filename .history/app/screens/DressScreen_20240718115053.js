...
// POZNÁMKA: Importujte useSubscription hook
...
const DressScreen = () => {
  ...
  // POZNÁMKA: Použijte useSubscription hook
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Fetching products with category: dress");
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      // Aktualizujte zbývající počet try-onů v kontextu
      ...
      if (error.response) {
        if (error.response.status === 403) {
          if (error.response.data.error === 'Your subscription is not active') {
            ...
            setError(error.response.data.error || 'Nepodařilo se zpracovat Try on. Zkuste to prosím znovu.');
          }
        } else {
          setError('Nepodařilo se zpracovat Try on. Zkuste to prosím znovu.');
        }
      } else if (error.request) {
        setError('Nepodařilo se spojit se serverem. Zkontrolujte své připojení.');
      } else {
