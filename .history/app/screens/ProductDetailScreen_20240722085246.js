...
const ProductDetailScreen = () => {
  ...
  const handleScroll = useCallback((event) => {
    ...
    const index = Math.round(contentOffset / (CONTAINER_WIDTH * 0.9));
    if (index !== selectedProfileIndex) {
      setSelectedProfileIndex(index);
      flatListRef.current.scrollToOffset({
        offset: index * CONTAINER_WIDTH * 0.9,
        animated: true,
      });
    }
  }, [selectedProfileIndex, setSelectedProfileIndex]);
  ...
  }, [navigation, userId, setSelectedProfileIndex]);
    ...
    <View style={styles.container}>
      <FlatList
        ...
        getItemLayout={(data, index) => ({
          length: CONTAINER_WIDTH * 0.9,
          offset: CONTAINER_WIDTH * 0.9 * index,
          index,
        })}
        initialScrollIndex={selectedProfileIndex}
        onScrollToIndexFailed={() => {}}
        snapToInterval={CONTAINER_WIDTH * 0.9}
...
const styles = StyleSheet.create({
  ...
  profileItemContainer: {
    ...
    justifyContent: 'center',
    alignItems: 'center',
    marginRight: CONTAINER_WIDTH * 0.1,
  },
  profileImage: {
    width: CONTAINER_WIDTH * 0.9,
    height: CONTAINER_HEIGHT,
