...
const NewScreen = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      if (route.params?.updatedSelectedProfileIndex !== undefined) {
        setSelectedProfileIndex(route.params.updatedSelectedProfileIndex);
        flatListRef.current?.scrollToIndex({
          index: route.params.updatedSelectedProfileIndex,
          animated: false,
          viewPosition: 0.5
        });
        navigation.setParams({ updatedSelectedProfileIndex: undefined });
      }
    }, [route.params?.updatedSelectedProfileIndex])
  );
