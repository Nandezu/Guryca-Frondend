...
const ImageDetailScreen = () => {
  ...
  const { imageUrl, profileIndex } = route.params;
  ...
  const handleDownload = async () => {
    try {
      ...
      if (freeSpace < 1024 * 1024) { // 1 MB
        Alert.alert('Error', 'Not enough free space to save the image');
        return;
      }
      ...
      if (downloadResult.status !== 200) {
        ...
        Alert.alert('Error', `Failed to download image. Status: ${downloadResult.status}`);
        return;
      }
  ...
  const handleDelete = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Delete', onPress: () => {
          navigation.navigate('DressScreen', { deletedProfileIndex: profileIndex });
        }}
      ]
    );
  };
