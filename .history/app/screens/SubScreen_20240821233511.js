...
const SubScreen = () => {
  ...
  const handleBuySubscription = async (planType, duration) => {
    try {
      const packageToPurchase = products.find(p => 
        p.product.title.toLowerCase().includes(planType.toLowerCase()) &&
        p.product.title.toLowerCase().includes(duration.toLowerCase())
      );
      ...
      await verifyPurchase(productIdentifier, customerInfo.originalAppUserId);
  ...
  const verifyPurchase = async (productIdentifier, appUserId) => {
    try {
      const token = await getToken();
