...
const ManageProfileImage = () => {
  ...
  const setActiveProfileImage = async (imageUrl) => {
    try {
      ...
      const response = await axios.post(`http://192.168.0.106:8000/user/users/${userId}/set_active_profile_image/`, 
        { image_url: imageUrl },
        {
          headers: {
            'Authorization': `Token ${userToken}`
  ...
  const renderProfileItem = ({ item, index }) => (
    <View style={styles.profileItemContainer}>
      ...
      <View style={styles.imageActions}>
        ...
        >
          <Text style={styles.actionButtonText}>{index === 0 ? 'Aktivní' : 'Nastavit jako hlavní'}</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          style={styles.actionButton} 
