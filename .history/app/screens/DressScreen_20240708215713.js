...
const DressScreen = () => {
  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      if (profileImage) {
        console.log("Setting profiles with profile image:", profileImage);
        setProfiles([{ id: 'profile', imageUrl: profileImage }, ...response.data.map(item => ({ id: item.id, imageUrl: item.result_image }))]);
      } else {
        console.warn('Profile image URL is not loaded yet');
        setProfiles([{ id: 'profile', imageUrl: null }]);
      }
    } catch (error) {
      console.error('Error loading try-on results:', error);
      setError('Nepodařilo se načíst výsledky try-on.');
      setProfiles([{ id: 'profile', imageUrl: profileImage || null }]);
    }
  };
