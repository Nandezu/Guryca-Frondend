...
const AIScreen = () => {
    ...
    useCallback(() => {
      const loadData = async () => {
        try {
          ...
          setError('Došlo k chybě při načítání dat. Prosím, zkuste to znovu později.');
        }
      };
  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      const results = [
        ...
        ...response.data
          .filter(item => item.result_image)
          .map(item => ({ id: item.id, imageUrl: item.result_image }))
      ];
      ...
      if (results.length > MAX_TRY_ON_RESULTS) {
        Alert.alert(
          'Dosažen limit zkoušení',
          'Dosáhli jste maximálního počtu zkoušení. Každé nové zkoušení vymaže nejstarší výsledek. Prosím, zálohujte si důležité výsledky.'
        );
      }
      ...
      setError('Nepodařilo se načíst výsledky zkoušení.');
  ...
  const fetchProducts = async () => {
    try {
      ...
      const shuffledProducts = shuffleArray([...response.data], seed);
      
      const limitedProducts = shuffledProducts.slice(0, 50);
    ...
    try {
      ...
      if (!useFeatureResponse.data.success) {
        if (useFeatureResponse.data.error === 'No virtual try-ons remaining') {
          Alert.alert('Žádné zbývající zkoušení', 'Pro získání dalších zkoušení upgradujte své předplatné nebo počkejte na měsíční obnovu, pokud máte prémiový plán.');
          setIsLoading(false);
          return;
        }
      ...
      if (response.data && response.data.result_image) {
        ...
        if (updatedProfiles.length > MAX_TRY_ON_RESULTS) {
          ...
          for (const profile of excessProfiles) {
            await axios.delete(`${BASE_URL}/tryon/delete-result/${profile.id}/`, {
              headers: { 'Authorization': `Token ${userToken}` }
            });
          }
      ...
      if (error.message === 'User not authenticated') {
        setError('Pro použití funkce zkoušení se prosím přihlaste.');
      } else if (error.response && error.response.status === 403) {
        setError('Vyčerpali jste všechna virtuální zkoušení. Pro získání dalších upgradujte své předplatné nebo počkejte na měsíční obnovu, pokud máte prémiový plán.');
      } else {
        setError('Nepodařilo se zpracovat zkoušení. Prosím, zkuste to znovu.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const renderProfileItem = ({ item, index }) => {
    const isFromImageDetail = item.imageUrl && item.imageUrl.includes('from_image_detail=true');
    return (
      <TouchableOpacity
        onPress={() => {
          if (index !== 0 && item.id !== 'profile' && item.imageUrl) {
            setSelectedProfileIndex(index);
        ...
        disabled={index === 0 || item.id === 'profile' || !item.imageUrl}
        delayPressIn={100}
        delayPressOut={100}
        delayLongPress={200}
      >
        <View style={styles.profileItemContainer}>
          {item.imageUrl ? (
            ...
            <View style={[styles.profileImage, { backgroundColor: 'gray' }]}>
              <Text style={styles.placeholderText}>Žádný obrázek</Text>
            </View>
          )}
        </View>
      </TouchableOpacity>
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('AIDetailScreen', { product, userId, initialIndex: selectedProfileIndex })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
    ...
    <View style={styles.container}>
      ...
      {products.length > 0 ? (
        ...
        <View style={styles.noProductsContainer}>
          <Text style={styles.noProductsText}>Momentálně nejsou k dispozici žádné produkty.</Text>
        </View>
      )}
      ...
      {isLoading && (
        <View style={styles.loadingOverlay}>
          ...
          />
          <Text style={styles.loadingText}>Oblékám....</Text>
        </View>
      )}
    </View>
  );
