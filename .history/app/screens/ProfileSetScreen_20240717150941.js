...
const ProfileSetScreen = () => {
  ...
  const handleLogout = async () => {
    try {
      const token = await AsyncStorage.getItem('userToken');
      if (token) {
        await axios.post('http://192.168.0.106:8000/user/logout/', {}, {
  ...
  return (
    <View style={styles.container}>
      ...
      {renderNewButton('Change Region', [styles.buttonStyle, { top: SCREEN_HEIGHT * 0.28 }], () => handleNavigation('ChangeRegionScreen'))}
      {renderNewButton('Change Email', [styles.buttonStyle, { top: SCREEN_HEIGHT * 0.36 }], () => handleNavigation('ChangeEmailScreen'))}
      {renderNewButton('Change Password', [styles.buttonStyle, { top: SCREEN_HEIGHT * 0.44 }], () => handleNavigation('ChangePasswordScreen'))}
      {renderNewButton('Subscription', [styles.buttonStyle, { top: SCREEN_HEIGHT * 0.56, backgroundColor: '#FFFF03' }], () => navigation.navigate('SubScreen', { userId }), { color: 'black' })}
      ...
      {renderNewButton('Logout', [styles.buttonStyle, { top: SCREEN_HEIGHT * 0.96, backgroundColor: '#CFD6DE' }], handleLogout, { color: 'black' })}
    </View>
  );
};
