  ...
  const addToProfileImages = async () => {
    try {
      ...
      if (error.response) {
        ...
        if (error.response.data.error && error.response.data.error.includes('No profile images remaining')) {
          setStatusMessage('Your subscription does not allow adding more profile images. Please upgrade your plan or delete an existing image.');
        } else {
