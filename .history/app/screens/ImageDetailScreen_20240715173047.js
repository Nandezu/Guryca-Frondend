...
const ImageDetailScreen = () => {
        ...
        { text: 'Zrušit', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (!userId) {
                console.error('userId je nedefinované');
                Alert.alert('Chyba', 'userId je nedefinované');
                return;
              }
  ...
  const addToProfileImages = async () => {
    try {
      ...
      Alert.alert('Chyba', `Nepodařilo se přidat obrázek do profilových obrázků: ${error.message}`);
    } finally {
      const fileUri = `${FileSystem.cacheDirectory}tempProfileImage.jpg`;
...
const styles = StyleSheet.create({
  ...
  addToProfileButton: {
    position: 'absolute',
    bottom: -SCREEN_HEIGHT * 0.125, // Upravte tuto hodnotu podle potřeby
    right: SCREEN_WIDTH * 0.05, // Upravte tuto hodnotu pro horizontální pozici
    width: SCREEN_WIDTH * 0.115, // Zvětšete šířku tlačítka
    height: SCREEN_WIDTH * 0.115, // Zvětšete výšku tlačítka
    justifyContent: 'center',
    alignItems: 'center',
  },
