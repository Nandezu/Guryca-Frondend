...
const DressScreen = () => {
  ...
  const [currentVisibleImage, setCurrentVisibleImage] = useState(null);
    ...
    useCallback(() => {
      const loadData = async () => {
        try {
          ...
          const tryOnResults = await loadTryOnResults();

          const allProfiles = [{ id: 'profile', imageUrl: profileImage }, ...tryOnResults];

          if (route.params?.deletedItemId) {
            const updatedResults = allProfiles.filter(item => item.id !== route.params.deletedItemId);
          ...
          setCurrentVisibleImage(profileImage);
  ...
  const loadTryOnResults = async () => {
    try {
      ...
      const results = response.data.map(item => ({ id: item.id, imageUrl: item.result_image }));
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      if (response.data && response.data.result_image) {
        const updatedProfiles = await loadTryOnResults();
        setProfiles([{ id: 'current', imageUrl: currentVisibleImage }, ...updatedProfiles]);
        if (flatListRef.current && updatedProfiles.length > 0) {
  ...
  const handleScroll = (event) => {
    ...
    if (profiles[index]) {
      setCurrentVisibleImage(profiles[index].imageUrl);
      console.log('Current visible image updated:', profiles[index].imageUrl);
    }
  };

  const renderProfileItem = ({ item, index }) => {
    console.log(`Rendering profile item: ${index}, imageUrl: ${item.imageUrl}`);
    return (
      <TouchableOpacity
        onPress={() => {
