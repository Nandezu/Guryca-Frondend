  ...
  const initializeIAP = async () => {
    try {
      ...
      if (availableProducts.length === 0) {
        ...
        // Zkusíme použít getProducts místo getSubscriptions
        const productsFromGetProducts = await RNIap.getProducts({ skus: productIds });
        console.log('Products from getProducts:', productsFromGetProducts);
        
        if (productsFromGetProducts.length > 0) {
          setProducts(productsFromGetProducts);
          console.log('Products set successfully from getProducts:', productsFromGetProducts);
        } else {
          console.warn('Still no products available after using getProducts');
        }
      } else {
      ...
      setIapInitialized(true); // Set to true even on error to allow UI to render
    }
  };
  ...
  const renderButtons = (plan) => {
    ...
    return (
      <View style={[styles.buttonsContainer, { bottom: BUTTONS_BOTTOM_OFFSET }]}>
        ...
        {!isCurrentPlan && !plan.isFree && iapInitialized && (
          <>
            <TouchableOpacity
