...
const ManageProfilePictures = () => {
  ...
  const fetchCurrentProfileImage = async () => {
    try {
      ...
      if (response.data.profile_image_url) {
        setCurrentProfileImage({ id: 'current', url: response.data.profile_image_url });
      }
    } catch (error) {
  ...
  const uploadPhoto = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        await fetchPhotos();
        await fetchCurrentProfileImage();  // Refresh current profile image
  ...
  const selectAsProfileImage = async (photoUrl) => {
    if (!photoUrl) {
      Alert.alert('Error', 'Cannot set an empty profile image');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={[currentProfileImage, ...photos].filter(item => item !== null)}
