...
const DressScreen = () => {
  ...
  useEffect(() => {
    ...
    const handleTryOn = async (product) => {
      ...
      try {
        ...
        setSelectedProfileIndex(profiles.length); // Vybere nově přidaný obrázek
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Zobrazení produktů a tlačítek "Try on" */}
      {products.map((product, index) => (
        <View key={index} style={styles.productContainer}>
          <Image source={{ uri: product.image_url }} style={styles.productImage} />
          <TouchableOpacity onPress={() => handleTryOn(product)} style={styles.tryOnButton}>
            <Text>Try On</Text>
          </TouchableOpacity>
        </View>
      ))}
...
const styles = StyleSheet.create({
  ...
  productContainer: {
    // TODO: Styly pro kontejner produktu
  },
  productImage: {
    // TODO: Styly pro obrázek produktu
  },
  tryOnButton: {
    // TODO: Styly pro tlačítko "Try on"
  },
  loadingContainer: {
    position: 'absolute',
    top: 0,
