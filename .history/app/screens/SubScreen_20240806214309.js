...
const SubScreen = () => {
  ...
  useEffect(() => {
    const subscription = InAppPurchases.addPurchaseListener(({ responseCode, results, errorCode }) => {
      if (responseCode === InAppPurchases.IAPResponseCode.OK) {
        results.forEach((purchase) => {
          if (!purchase.acknowledged) {
            InAppPurchases.finishTransactionAsync(purchase, true);
            updateBackendSubscription(purchase);
          }
        });
      }
    });
  ...
  const loadProducts = async () => {
    try {
      const productIds = Platform.OS === 'ios' ? Object.values(PRODUCT_IDS.iOS) : Object.values(PRODUCT_IDS.android);
  ...
  const handleBuySubscription = async (planType, duration) => {
    ...
    if (!productId) {
      Alert.alert('Error', 'This product is not available for purchase.');
      return;
    }
  ...
  const getProductId = (planType, duration) => {
    const platform = Platform.OS === 'ios' ? PRODUCT_IDS.iOS : PRODUCT_IDS.android;
    const key = `${planType.toUpperCase()}_${duration.toUpperCase()}`;
    return platform[key];
  };

  const updateBackendSubscription = async (purchase) => {
    try {
      const token = await getToken();
      await axios.post(`${BASE_URL}/subscription/update/`, {
  ...
  const renderButtons = (plan) => {
    ...
    const planProducts = products.filter(p => p.productId.toLowerCase().includes(plan.name.toLowerCase().replace(' plan', '')));
          ...
          <>
            {planProducts.map((product) => (
              <TouchableOpacity
                ...
                onPress={() => handleBuySubscription(plan.name, product.productId.includes('annual') ? 'annual' : 'monthly')}
              >
                <Text style={styles.orderButtonText}>{`BUY ${product.title} (${product.priceString})`}</Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  freePlanImage: {
    ...
    marginTop: SCREEN_HEIGHT * 0.050, // Adjust top margin to center the larger image
  },
  buttonsContainer: {
    position: 'absolute',
    left: 0,
