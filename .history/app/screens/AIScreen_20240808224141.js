  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      if (results.length > MAX_TRY_ON_RESULTS) {
        Alert.alert(
          'Test limit reached.',
          'You have reached the maximum number of attempts. Each new attempt will erase the oldest result. Please back up important results.'
        );
      }
    ...
    try {
      ...
      if (!useFeatureResponse.data.success) {
        if (useFeatureResponse.data.error === 'No virtual try-ons remaining') {
          Alert.alert('No remaining attempts. To get more attempts, upgrade your subscription or wait for the monthly renewal if you have a premium plan.');
          setIsLoading(false);
          return;
        }
      ...
      if (error.message === 'User not authenticated') {
        ...
        setError('You have exhausted all virtual tests. To get more, upgrade your subscription or wait for the monthly renewal if you have a premium plan.');
      } else {
        setError('The try on processing failed. Please try again.');
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <View style={styles.container}>
      ...
      {products.length > 0 ? (
        ...
        <View style={styles.noProductsContainer}>
          <Text style={styles.noProductsText}>I'm thinking about what you might like..</Text>
        </View>
      )}
