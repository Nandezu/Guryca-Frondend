...
const SubScreen = () => {
  ...
  const handleBuySubscription = (duration) => {
    // Implementujte logiku pro nákup předplatného
    console.log(`Buying ${duration} subscription`);
  };
        ...
        {isCurrentPlan && !plan.isFree && (
          <>
            ...
            {!isCancelled ? (
              ...
              <>
                <TouchableOpacity style={[styles.orderButton, { backgroundColor: '#000' }]} onPress={() => handleBuySubscription('month')}>
                  <Text style={styles.orderButtonText}>BUY MONTH</Text>
                </TouchableOpacity>
                <Text style={styles.discountText}>20% discount on yearly plan</Text>
                <TouchableOpacity style={[styles.orderButton, { backgroundColor: '#000' }]} onPress={() => handleBuySubscription('year')}>
