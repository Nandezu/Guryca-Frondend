...
const SubScreen = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      const loadCancelStatus = async () => {
        try {
          const cancelStatus = await AsyncStorage.getItem('isCancelled');
          if (cancelStatus !== null) {
            setIsCancelled(JSON.parse(cancelStatus));
          }
        } catch (error) {
          console.error('Error loading cancel status from AsyncStorage:', error);
        }
      };
  ...
  const confirmCancelSubscription = async () => {
    try {
      ...
      setIsCancelled(true);
      await AsyncStorage.setItem('isCancelled', JSON.stringify(true)); // Uložit stav do AsyncStorage
