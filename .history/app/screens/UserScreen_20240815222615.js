...
const UserScreen = () => {
  console.log('UserScreen is rendering');
    ...
    useCallback(() => {
      ...
      const loadData = async () => {
        try {
          setIsLoading(true);
          if (!userId) {
            console.log('userId is not available, displaying only the hub');
            setIsLoading(false);
            return;
          }
          ...
          setError('Error loading data: ' + (error.message || 'Unknown mistake'));
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const loadProfileImage = async () => {
    try {
      ...
      console.log("Loaded URL of active profile picture:", activeImageUrl);
  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      console.error('Error loading try-on results:', error.response ? error.response.data : error.message);
