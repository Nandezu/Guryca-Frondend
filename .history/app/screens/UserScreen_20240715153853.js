...
const UserScreen = () => {
  ...
  const renderProfileItem = ({ item, index }) => {
    const isMainProfileImage = index === 0;
    return (
      <TouchableOpacity
        onPress={() => {
        ...
        <View style={styles.profileItemContainer}>
          {item.imageUrl ? (
            <Image
              ...
              style={[
                styles.profileImage,
                isMainProfileImage && styles.transformedProfileImage // Apply transformedProfileImage style only to the main profile image
              ]}
              resizeMode="cover"
            />
          ) : (
