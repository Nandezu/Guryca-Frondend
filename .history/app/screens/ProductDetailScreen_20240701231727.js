...
const ProductDetailScreen = () => {
  ...
  useEffect(() => {
    ...
    const checkIfFavorite = async () => {
      try {
        const token = await AsyncStorage.getItem('userToken');
        const response = await axios.get(`http://192.168.0.106:8000/favorites/?user=${userId}&product=${product.id}`, {
          headers: {
            'Authorization': `Token ${token}`
          }
        });
  ...
  const handleLikePress = async () => {
    try {
      ...
      if (isFavorite) {
        await axios.delete(`http://192.168.0.106:8000/favorites/toggle/`, { data: { product: product.id }, ...config });
      } else {
        await axios.post('http://192.168.0.106:8000/favorites/toggle/', { product: product.id }, config);
      }
