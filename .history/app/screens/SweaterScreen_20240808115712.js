...
const SweaterScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const cachedProducts = await AsyncStorage.getItem('cachedSweaterProducts');
      ...
      if (!lastShuffleTime || now - parseInt(lastShuffleTime) > 1 * 60 * 1000 || !cachedProducts) {
        ...
        const response = await axios.get(`${BASE_URL}/products/`, {
          ...
          params: { clothing_category: 'sweater' }
        });
        ...
        await AsyncStorage.setItem('cachedSweaterProducts', JSON.stringify(selectedProducts));
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('SweaterDetailScreen', { product, userId, initialIndex: selectedProfileIndex })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
  ...
  return (
    <View style={styles.container}>
      ...
      {renderImageWithTouchableOpacity(
        require('../../assets/images/sweaterg.png'),
        styles.dressImage,
        styles.dressButton,
        'first',
