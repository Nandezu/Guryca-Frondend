...
const NewScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const lastShuffleTime = await AsyncStorage.getItem('lastProductShuffleTime');
      const storedProducts = await AsyncStorage.getItem('shuffledProducts');

      if (lastShuffleTime && now - parseInt(lastShuffleTime) < 24 * 60 * 60 * 1000 && storedProducts) {
        console.log("Using cached shuffled products");
        setProducts(JSON.parse(storedProducts));
        return;
      }

      console.log("Fetching and shuffling new products");
      ...
      // Uložení zamíchaných produktů a času do AsyncStorage
      await AsyncStorage.setItem('lastProductShuffleTime', now.toString());
      await AsyncStorage.setItem('shuffledProducts', JSON.stringify(shuffledProducts));
