...
import { useProfile } from '../../ProfileProvider';  // Import useProfile
...
const AIScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Vytvoříme seed pro pseudonáhodné míchání
      const seed = getWeeklySeed(userId);
      
      // Promícháme všechny produkty
      const shuffledProducts = shuffleArray([...response.data], seed);
      
      // Vybereme prvních 50 produktů
      const limitedProducts = shuffledProducts.slice(0, 50);
      
      // Nastavíme omezený počet produktů do stavu
    ...
    try {
      ...
      if (response.data && response.data.result_image) {
        ...
        if (flatListRef.current && updatedProfiles.length > 1) {
          flatListRef.current.scrollToOffset({ offset: SCREEN_WIDTH * selectedProfileIndex, animated: true });
        }
      } else {
      ...
      <TouchableOpacity
        onPress={() => {
          if (index !== 0 && item.id !== 'profile') {
            setSelectedProfileIndex(index); // Aktualizace indexu při výběru
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        extraData={selectedProfileIndex} // Použití selectedProfileIndex jako extraData
