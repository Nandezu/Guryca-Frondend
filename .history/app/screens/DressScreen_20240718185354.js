...
const DressScreen = () => {
  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      // Limit try-on results to 20
      if (results.length > MAX_TRY_ON_RESULTS) {
        results.splice(MAX_TRY_ON_RESULTS);
        Alert.alert('Try-on Limit Reached', 'You have reached the maximum number of try-ons. Please delete some try-ons to make space for new ones.');
      }
  ...
  const handleTryOn = async (product) => {
    ...
    try {
        ...
        // NOTE: Implement API call for using try-on
        ...
        if (!useFeatureResponse.data.success) {
            // NOTE: If try-ons are exhausted, show a message and exit the function
        ...
        // NOTE: Update remaining try-ons in context
