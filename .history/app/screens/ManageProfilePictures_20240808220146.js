...
const ManageProfileImage = () => {
  ...
  const uploadPhoto = async (uri) => {
    try {
      ...
      const response = await axios.post(`${BASE_URL}/user/users/${userId}/upload_profile_image/?is_new_upload=true`, formData, {
        headers: {
  ...
  const deleteProfileImage = async (imageUrl) => {
    try {
      ...
      const response = await axios.delete(`${BASE_URL}/user/users/${userId}/remove_profile_image/`,
        {
          headers: {
            'Authorization': `Token ${userToken}`
          },
  ...
  return (
    <View style={styles.container}>
      {renderImageWithTouchableOpacity(
        ...
        { width: SCREEN_WIDTH * 0.35, height: SCREEN_HEIGHT * 0.16 },
        'nandezu',
        { position: 'absolute', top: SCREEN_HEIGHT * -0.005, left: SCREEN_WIDTH * 0.330, zIndex: 2 },
        'UserScreen'
      )}
      {renderImageWithTouchableOpacity(
