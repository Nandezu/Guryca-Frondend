...
const NewDetailScreen = () => {
  ...
  const { product, userId, selectedProfileIndex } = route.params;
  const flatListRef = useRef(null);

  const { profiles, setProfiles, setSelectedProfileIndex } = useProfile();
  ...
  const onViewableItemsChanged = useCallback(({ viewableItems }) => {
    if (viewableItems.length > 0) {
      ...
      navigation.setParams({ selectedProfileIndex: newIndex });
    }
