...
export default function SignUpScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (nickname.length > 20) {
      setMessage('Nickname cannot be more than 20 characters.');
      return;
    }
    ...
    if (password.length < 7 || !/\d/.test(password) || !/[A-Z]/.test(password)) {
      setMessage('Password must be at least 7 characters long, contain at least one number, and one uppercase letter.');
      return;
    }
      ...
      {!message && (
        <View style={styles.termsContainer}>
          <CheckBox
            ...
            tintColors={{ true: '#1E90FF', false: '#fff' }}
          />
          <Text style={styles.termsText} onPress={() => Linking.openURL('https://www.nandezu.com/terms-and-conditions')}>
