...
const SubScreen = () => {
  ...
  const fetchSubscriptionDetails = useCallback(async () => {
    try {
      ...
      // Kontrola, zda is_cancelled existuje a není undefined
      if ('is_cancelled' in response.data) {
        setIsCancelled(response.data.is_cancelled);
        // Uložte stav zrušení do AsyncStorage pouze pokud není undefined
        await AsyncStorage.setItem('isCancelled', JSON.stringify(response.data.is_cancelled));
      } else {
        console.warn('is_cancelled property not found in response data');
        // Možná byste chtěli nastavit výchozí hodnotu
        setIsCancelled(false);
        await AsyncStorage.setItem('isCancelled', JSON.stringify(false));
      }
  ...
  const confirmCancelSubscription = async () => {
    try {
      ...
      setIsCancelled(true);
      await AsyncStorage.setItem('isCancelled', JSON.stringify(true));
      updateSubscriptionLimits(response.data.subscription_details);
      
      // Okamžitě aktualizujte stav předplatného
