...
const ImageDetailScreen = () => {
  ...
  const addToProfileImages = async () => {
    try {
      ...
      const scaledWidth = imageInfo.width * 0.87;
      const scaledHeight = (scaledWidth / imageInfo.width) * imageInfo.height;
      ...
      const markedImageUrl = manipResult.uri + '?from_image_detail=true';

      const formData = new FormData();
      formData.append('profile_image', {
        uri: Platform.OS === 'ios' ? markedImageUrl.replace('file://', '') : markedImageUrl,
        type: 'image/jpeg',
