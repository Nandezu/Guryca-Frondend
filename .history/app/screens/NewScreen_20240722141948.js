...
const NewScreen = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      if (route.params?.updatedProfileIndex !== undefined) {
        console.log('Updating selectedProfileIndex on focus:', route.params.updatedProfileIndex);
        setSelectedProfileIndex(route.params.updatedProfileIndex);
        flatListRef.current?.scrollToIndex({ 
          index: route.params.updatedProfileIndex, 
          animated: false,
          viewPosition: 0.5
        });
        navigation.setParams({ updatedProfileIndex: undefined });
      }
    }, [route.params?.updatedProfileIndex])
  );
  ...
  const handleScroll = (event) => {
    ...
    if (index !== selectedProfileIndex) {
      console.log('Updating selectedProfileIndex on scroll:', index);
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('NewDetailScreen', { 
            product, 
            userId, 
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) => item.sku || `product-${index}`}
