...
const NewDetailScreen = () => {
  ...
  const { product, userId, initialIndex, updateSelectedIndex } = route.params;
  ...
  useEffect(() => {
    ...
    setSelectedProfileIndex(initialIndex);
  }, [initialIndex]);

  useFocusEffect(
    useCallback(() => {
      return () => {
        // Toto se spustí při opuštění obrazovky
        updateSelectedIndex(selectedProfileIndex);
      };
    }, [selectedProfileIndex, updateSelectedIndex])
  );
