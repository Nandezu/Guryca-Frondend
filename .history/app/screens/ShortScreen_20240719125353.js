...
const ShortScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Fetching products with category: short");
      const response = await axios.get('http://192.168.0.106:8000/products/', {
        headers: {
          'Authorization': `Token ${userToken}`
        },
        params: {
          clothing_category: 'short'
        }
      });
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('ShortsDetailScreen', { product, userId })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
  ...
  onViewableItemsChanged={onViewableItemsChanged}
     ...
     />
      ...
      {renderImageWithTouchableOpacity(
        require('../../assets/images/shortsg.png'),
        styles.dressImage,
        styles.dressButton,
        'first',
