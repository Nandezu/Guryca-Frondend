...
const SubScreen = () => {
  ...
  const fetchSubscriptionDetails = useCallback(async () => {
    try {
      ...
      setShowCancelButton(!response.data.is_cancelled);
      updateSubscriptionLimits(response.data);

      // Reset showCancelButton if a new subscription is detected
      const storedSubscription = await AsyncStorage.getItem('currentSubscription');
      if (storedSubscription !== response.data.subscription_type) {
        setShowCancelButton(true);
        await AsyncStorage.setItem('currentSubscription', response.data.subscription_type);
      }
    } catch (error) {
  ...
  const renderButtons = (plan) => {
    ...
    return (
      <View style={[styles.buttonsContainer, { bottom: BUTTONS_BOTTOM_OFFSET }]}>
        ...
        {isCurrentPlan && !plan.isFree && showCancelButton && (
        ...
        {(!isCurrentPlan || (isCurrentPlan && isCancelled)) && !plan.isFree && (
          <>
