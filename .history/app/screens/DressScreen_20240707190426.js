...
const DressScreen = () => {
  ...
  const fetchProfileImage = async () => {
    try {
      ...
      setProfiles([{ url: profileImageUrl }]);
      await AsyncStorage.setItem('profiles', JSON.stringify([{ url: profileImageUrl }]));
    } catch (error) {
      console.error(error);
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      const currentImageUrl = profiles[selectedProfileIndex].url;
      ...
      if (response.data && response.data.result_image) {
        const newImage = response.data.result_image;
        Image.getSize(newImage, (width, height) => {
          const aspectRatio = width / height;
          const newHeight = CONTAINER_HEIGHT;
          ...
          manageProfiles(imageWithMetadata);
          setSelectedProfileIndex(profiles.length);
          flatListRef.current.scrollToEnd({ animated: true });
        }, (error) => {
          console.error('Nepodařilo se získat rozměry obrázku', error);
          setError('Nepodařilo se zpracovat výsledný obrázek');
        });
      } else {
