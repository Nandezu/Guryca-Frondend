...
const UserScreen = () => {
  ...
  const { userId, profiles: initialProfiles, selectedProfileIndex: initialSelectedProfileIndex } = route.params || {};
  const flatListRef = useRef(null);
  const [profileImageUrl, setProfileImageUrl] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const [profiles, setProfiles] = useState(initialProfiles || []);
  const [selectedProfileIndex, setSelectedProfileIndex] = useState(initialSelectedProfileIndex || 0);
    ...
    useCallback(() => {
      ...
      const loadData = async () => {
        try {
          ...
          if (initialProfiles) {
            console.log('Použití předaných profilů');
          ...
          if (route.params?.deletedItemId) {
            const updatedResults = profiles.filter(item => item.id !== route.params.deletedItemId);
      ...
      loadData();
    }, [userId, route.params?.deletedItemId, initialProfiles])
  );

  useEffect(() => {
    if (initialSelectedProfileIndex !== undefined) {
      setSelectedProfileIndex(initialSelectedProfileIndex);
      flatListRef.current?.scrollToIndex({ 
        index: initialSelectedProfileIndex, 
        animated: false,
        viewPosition: 0.5
      });
    }
  }, [initialSelectedProfileIndex]);
