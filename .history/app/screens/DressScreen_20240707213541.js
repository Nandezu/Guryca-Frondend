...
const NARROW_FACTOR = 0.8; // Faktor zúžení pro try-on obrázky
...
const DressScreen = () => {
  ...
  const manageProfiles = async (newProfile) => {
    setProfiles(prevProfiles => {
      const updatedProfiles = [...prevProfiles, { type: 'try-on', url: newProfile }];
  ...
  const renderProfileItem = ({ item }) => (
    <View style={styles.profileItemContainer}>
      <Image
        ...
        style={[
          styles.profileImage,
          item.type === 'try-on' && { width: CONTAINER_WIDTH * NARROW_FACTOR }
        ]}
        resizeMode={item.type === 'profile' ? 'cover' : 'contain'}
      />
    </View>
  );
