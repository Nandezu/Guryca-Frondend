...
const DressScreen = () => {
  ...
  const loadProfiles = async () => {
    try {
      ...
      setProfiles(response.data.map(item => item.result_image));
    } catch (error) {
      console.error('Error loading profiles:', error);
      if (error.response && error.response.status === 401) {
        setError('Pro zobrazení výsledků try-on se prosím přihlaste.');
      } else {
  ...
  const handleDelete = async (index) => {
    try {
      await axios.delete(`http://192.168.0.106:8000/tryon/delete-result/${index}/`);
      loadProfiles();
    } catch (error) {
  ...
  const renderProfileItem = ({ item, index }) => (
    ...
    >
      <View style={styles.profileItemContainer}>
        <Image
          source={{ uri: item }}
          style={[styles.profileImage, styles.transformedProfileImage]}
          resizeMode="cover"
        />
      </View>
    </TouchableOpacity>
