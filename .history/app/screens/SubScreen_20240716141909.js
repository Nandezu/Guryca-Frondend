...
const BUTTON_CONFIG = {
  ...
  bottomOffset: SCREEN_HEIGHT * 0.03, // Vzdálenost tlačítek od spodní části obrazovky
};
      ...
      {!isFree && (
        <View style={[styles.buttonsContainer, { bottom: BUTTON_CONFIG.bottomOffset }]}>
          <TouchableOpacity style={[styles.orderButton, { backgroundColor: '#FFC05F' }]}>
            <Text style={styles.orderButtonText}>BUY MONTH PLAN</Text>
          </TouchableOpacity>
          <Text style={styles.discountText}>20% discount on yearly plan</Text>
          <TouchableOpacity style={[styles.orderButton, { backgroundColor: '#FFC05F' }]}>
            <Text style={styles.orderButtonText}>BUY YEAR PLAN</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
