...
<View style={[styles.imageWrapper, styles.dressContainer]}>
  <Image
    source={require('../../assets/images/dressg.png')}
    style={[styles.image, styles.dressImage, pressed['first'] && styles.pressedImage]}
    resizeMode="contain"
  />
  <TouchableOpacity
    style={[styles.button, styles.dressButton]}
    onPressIn={() => {
      let pressTimer = setTimeout(() => setPressed({ ...pressed, 'first': true }), 50); // Reduced delay for dressg button
    }}
    onPressOut={() => {
      clearTimeout(pressTimer);
      setPressed({ ...pressed, 'first': false });
    }}
    onPress={() => {
      if (pressed['first']) {
    ...
    delayPressIn={50}  // Reduced delay before press in starts for dressg button
    delayPressOut={50} // Reduced delay before press out starts for dressg button
    delayLongPress={100} // Delay for long press
  />
</View>
