...
const LeggingsScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const cachedProducts = await AsyncStorage.getItem('cachedLeggingsProducts');
      ...
      if (!lastShuffleTime || now - parseInt(lastShuffleTime) > 1 * 60 * 1000 || !cachedProducts) {
        ...
        const response = await axios.get(`${BASE_URL}/products/`, {
          ...
          params: { clothing_category: 'leggings' }
        });
        ...
        await AsyncStorage.setItem('cachedLeggingsProducts', JSON.stringify(selectedProducts));
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('LegginsDetailScreen', { product, userId, initialIndex: selectedProfileIndex })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
