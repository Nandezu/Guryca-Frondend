  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.0.106:8000/tryon/try-on/', {
        ...
        reference_image: profiles[selectedProfileIndex].imageUrl, // Use the current image URL
      }, {
        headers: {
          'Authorization': `Token ${userToken}`
        }
      ...
      if (response.data && response.data.result_image) {
        const updatedProfiles = await loadTryOnResults(profiles[selectedProfileIndex].imageUrl); // Update with the current image URL
