...
const SubScreen = () => {
  ...
  useEffect(() => {
    const initializeIAP = async () => {
      try {
        console.log('Initializing RNIap...');
        ...
        if (products.length === 0) {
          console.warn('No products found. Check your product IDs and Google Play Console setup.');
        } else {
    ...
    return () => {
      console.log('Ending RNIap connection');
      RNIap.endConnection();
    };
  }, []);
  ...
  const handleBuySubscription = async (planType, duration) => {
    try {
      ...
      console.log(`Attempting to buy subscription: ${planType} - ${duration} on ${device}`);
      ...
      console.log('Subscription details:', JSON.stringify(subscriptions, null, 2));

      if (subscriptions.length === 0) {
        console.error('No subscriptions found for this product ID');
        throw new Error('No subscriptions available');
      }

      const subscription = subscriptions[0];
      console.log('Subscription offers:', JSON.stringify(subscription.subscriptionOfferDetails, null, 2));

      if (!subscription.subscriptionOfferDetails || subscription.subscriptionOfferDetails.length === 0) {
        console.error('No subscription offers found for this product.');
  ...
  const verifyPurchase = async (productId, purchase) => {
    try {
      ...
      console.log('Verification response:', JSON.stringify(verificationResponse.data, null, 2));
      ...
      if (verificationResponse.data.success) {
        ...
        console.error('Failed to verify purchase on backend');
