...
const JumpsuitScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Fetching products with category: jumpsuit");
      const response = await axios.get('http://192.168.0.106:8000/products/', {
        headers: {
          'Authorization': `Token ${userToken}`
        },
        params: {
          clothing_category: 'jumpsuit'
        }
      });
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('JumpsuitDetailScreen', { product, userId })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
  ...
  onViewableItemsChanged={onViewableItemsChanged}
     ...
     />
      ...
      {renderImageWithTouchableOpacity(
        require('../../assets/images/jumpsuitg.png'),
        styles.dressImage,
        styles.dressButton,
        'first',
