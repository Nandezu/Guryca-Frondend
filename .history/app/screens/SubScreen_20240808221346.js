...
const SubScreen = () => {
  ...
  const handleBuySubscription = async (planType, duration) => {
    try {
      ...
      // Získejte URL pro platbu z vašeho backendu
      ...
      if (response.data.paymentUrl) {
        // Otevřete platební URL v prohlížeči
        await Linking.openURL(response.data.paymentUrl);
        
        // Zobrazit instrukce uživateli
        Alert.alert(
          'Payment Initiated',
          'Please complete the payment in your browser. After completion, return to the app and we will verify your purchase.',
          [
            {
              text: 'OK',
              onPress: async () => {
                // Zde implementujeme logiku pro ověření nákupu
  ...
  const verifyPurchase = async (productId) => {
    try {
      const token = await getToken();
