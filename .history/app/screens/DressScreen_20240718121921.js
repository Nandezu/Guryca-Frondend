...
const DressScreen = () => {
  ...
  const { updateSubscriptionLimits, subscriptionDetails } = useSubscription();
  ...
  useFocusEffect(
    useCallback(() => {
      const loadData = async () => {
        try {
          if (!userId) {
            throw new Error('userId is undefined in loadData');
          }
  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      throw new Error('Failed to load try-on results');
    }
  };
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      updateSubscriptionLimits(useFeatureResponse.data.subscription_details);
      ...
      if (error.response) {
        if (error.response.status === 403) {
          if (error.response.data.error === 'Your subscription is not active') {
            Alert.alert('Subscription Inactive', 'Your subscription is not active. Please renew your subscription.');
          } else if (error.response.data.error === 'No virtual try-ons remaining') {
            Alert.alert('No Try-Ons Remaining', 'Please upgrade your subscription to get more try-ons.');
          } else {
            setError(error.response.data.error || 'Failed to process Try on. Please try again.');
          }
        } else {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.subscriptionInfoContainer}>
        <Text style={styles.subscriptionInfoText}>
          Try-ons remaining: {subscriptionDetails.virtual_try_ons_remaining}
        </Text>
      </View>
    </View>
  );
