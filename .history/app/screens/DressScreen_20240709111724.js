    ...
    useCallback(() => {
      const loadData = async () => {
        try {
          ...
          let tryOnResults = await loadTryOnResults(profileImage);
          
          if (route.params?.deletedItemId) {
            tryOnResults = tryOnResults.filter(item => item.id !== route.params.deletedItemId && item.id !== 'profile');
            tryOnResults.unshift({ id: 'profile', imageUrl: profileImage });
