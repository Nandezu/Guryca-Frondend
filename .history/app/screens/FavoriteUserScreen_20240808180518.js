...
const FavouriteUserScreen = () => {
  ...
  useFocusEffect(
    ...
    }, [userId, route.params?.deletedItemId, route.params?.profiles])
  );
  ...
  const fetchProducts = async () => {
    try {
      ...
      const favoriteProducts = sortedFavorites.map(favorite => favorite.product);
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/tryon/results/`, {
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('MeProductDetailScreen', { 
            product, 
            userId, 
    ...
    <View style={styles.container}>
      ...
      {isLoading && (
        <View style={styles.loadingOverlay}>
          ...
          />
          <Text style={styles.errorText}>Dressing....</Text>
        </View>
      )}
    </View>
  );
