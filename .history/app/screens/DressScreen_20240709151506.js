...
const DressScreen = () => {
  ...
  useFocusEffect(
    useCallback(() => {
        ...
        try {
          ...
          if (profileImage) {
            ...
            if (route.params?.deletedItemId) {
              ...
              setProfiles([{ id: 'profile', imageUrl: profileImage }, ...updatedResults]);
              navigation.setParams({ deletedItemId: undefined });
            } else {
              setProfiles([{ id: 'profile', imageUrl: profileImage }, ...tryOnResults]);
            }
          } else {
            setProfiles([]);
          }
      ...
      loadData();
    }, [])  // Spustí se při každém zaměření na obrazovku
  );
