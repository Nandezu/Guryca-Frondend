...
const UserScreen = () => {
  ...
  const renderProfileItem = ({ item, index }) => {
    const isProfileImage = item.id === 'profile';

    return (
      <TouchableOpacity
        onPress={() => {
          if (!isProfileImage && item.id !== 'profile') {
        ...
        disabled={isProfileImage || item.id === 'profile'}
        delayPressIn={100}
        delayPressOut={100}
        delayLongPress={200}
      >
        <View style={styles.profileItemContainer}>
          {item.imageUrl ? (
            <Image
              ...
              style={[styles.profileImage, isProfileImage && styles.transformedProfileImage]}
              resizeMode="cover"
            />
          ) : (
