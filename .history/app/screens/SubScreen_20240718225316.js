...
const SubScreen = () => {
  ...
  const fetchSubscriptionDetails = useCallback(async () => {
    try {
      ...
      const cancelledStatus = await AsyncStorage.getItem('isCancelled');
      setIsCancelled(cancelledStatus === 'true' || response.data.is_cancelled);
  ...
  const handleCancelSubscription = async () => {
    try {
      ...
      await AsyncStorage.setItem('isCancelled', 'true');
    } catch (error) {
