...
const DressScreen = () => {
    ...
    useCallback(() => {
      const loadData = async () => {
        try {
          ...
          setCurrentVisibleImage(profileImage); // Nastavte počáteční hodnotu
  ...
  const loadTryOnResults = async (currentImage) => {
    try {
      const userToken = await getToken();
      if (!userToken) {
        throw new Error('User not authenticated');
      ...
      const results = [
        { id: 'current', imageUrl: currentImage },
      ...
      return [{ id: 'current', imageUrl: currentImage || null }];
    }
  };
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.0.106:8000/tryon/try-on/', {
        ...
        image_url: currentVisibleImage, // Použijte currentVisibleImage místo profileImageUrl
      }, {
        headers: {
          'Authorization': `Token ${userToken}`
        }
      ...
      if (response.data && response.data.result_image) {
        const updatedProfiles = await loadTryOnResults(currentVisibleImage);
        setProfiles(updatedProfiles);
        if (flatListRef.current && updatedProfiles.length > 1) {
          flatListRef.current.scrollToOffset({ offset: SCREEN_WIDTH * (updatedProfiles.length - 1), animated: true });
        }
      } else {
  ...
  const handleScroll = (event) => {
    ...
    // Aktualizujte currentVisibleImage
    if (profiles[index]) {
