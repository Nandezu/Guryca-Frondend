...
// Importujte useSubscription hook
...
const MAX_TRY_ON_RESULTS = 100;  // Maximum number of try-on results to keep

const SweatpantsScreen = () => {
  ...
  const loadTryOnResults = async (profileImage) => {
    try {
      ...
      // Check if the number of results exceeds the maximum limit
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Fetching products with category: sweatpants");
      const response = await axios.get('http://192.168.0.106:8000/products/', {
        headers: {
          'Authorization': `Token ${userToken}`
        },
        params: {
          clothing_category: 'sweatpants'
        }
      });
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      // Implement API call for using try-on
      ...
      if (!useFeatureResponse.data.success) {
        // If try-ons are exhausted, show a message and exit the function
      ...
      // Update remaining try-ons in context
      ...
      if (response.data && response.data.result_image) {
        ...
        // Check if the number of results exceeds the maximum limit and delete the oldest if necessary
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('SweatpantsDetailScreen', { product, userId })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
  ...
  onViewableItemsChanged={onViewableItemsChanged}
     ...
     />
      ...
      {renderImageWithTouchableOpacity(
        require('../../assets/images/sweatpantsg.png'),
        styles.dressImage,
        styles.dressButton,
        'first',
...
export default SweatpantsScreen;
