...
const DressScreen = () => {
  ...
  useEffect(() => {
    if (route.params?.deletedProfileIndex !== undefined) {
      const newProfiles = [...profiles];
      newProfiles.splice(route.params.deletedProfileIndex, 1);
      setProfiles(newProfiles);
      AsyncStorage.setItem('profiles', JSON.stringify(newProfiles));
      navigation.setParams({ deletedProfileIndex: undefined });
    }
  }, [route.params?.deletedProfileIndex, profiles]);
  ...
  snapToInterval={SCREEN_WIDTH} // Přidáno
  decelerationRate="fast" // Přidáno
  snapToAlignment="center" // Přidáno
/>
...
<ScrollView
  ...
  snapToInterval={SCREEN_WIDTH * 0.75} // Přidáno
  decelerationRate="fast" // Přidáno
  snapToAlignment="center" // Přidáno
>
