...
export default function AfterRegistration() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/user/pre-register/`, {
        username: nickname,
        email,
        password,
      ...
      if (response.status === 201) {
        setMessage('Pre-registration successful! Please check your email for confirmation code.');
        setTimeout(() => {
          navigation.navigate('CodeConfirmScreen', { email: email });
        }, 3000);
      } else {
        setMessage('Pre-registration failed. Please try again.');
      }
    } catch (error) {
      if (error.response) {
        setMessage('Pre-registration failed: ' + JSON.stringify(error.response.data));
