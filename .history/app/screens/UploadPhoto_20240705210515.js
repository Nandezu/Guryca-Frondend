...
const MAX_CONTAINER_WIDTH = SCREEN_WIDTH * 0.9;
const MAX_CONTAINER_HEIGHT = SCREEN_HEIGHT * 0.7;
...
export default function UploadPhoto() {
  ...
  const selectPhoto = async () => {
    ...
    if (pickerResult.assets && pickerResult.assets.length > 0) {
      ...
      Image.getSize(pickerResult.assets[0].uri, (width, height) => {
  ...
  const containerStyle = () => {
    if (imageSize.width === 0 || imageSize.height === 0) {
      return { width: MAX_CONTAINER_WIDTH, height: MAX_CONTAINER_HEIGHT };
    }

    const aspectRatio = imageSize.width / imageSize.height;
    let containerWidth = MAX_CONTAINER_WIDTH;
