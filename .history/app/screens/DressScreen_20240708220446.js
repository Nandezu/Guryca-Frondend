...
const DressScreen = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        await loadTryOnResults(profileImage);
        await fetchProducts();
      } catch (error) {
  ...
  const loadTryOnResults = async () => {
    try {
      ...
      // Čekáme na načtení profilového obrázku, pokud ještě není načtený
      if (!profileImageUrl) {
        await new Promise(resolve => {
          const checkProfileImage = () => {
            if (profileImageUrl) {
              ...
              setTimeout(checkProfileImage, 100);
            }
          };
          checkProfileImage();
        });
      ...
      console.log("Setting profiles with profile image:", profileImageUrl);
      setProfiles([{ id: 'profile', imageUrl: profileImageUrl }, ...response.data.map(item => ({ id: item.id, imageUrl: item.result_image }))]);
    } catch (error) {
      console.error('Error loading try-on results:', error);
      setError('Nepodařilo se načíst výsledky try-on.');
      setProfiles([{ id: 'profile', imageUrl: profileImageUrl || null }]);
    }
  };
  ...
  const handleTryOn = async (product) => {
    ...
    try {
      ...
      if (response.data && response.data.result_image) {
        ...
        setTimeout(() => {
          if (flatListRef.current && profiles.length > 1) {
