...
const ManageProfileImage = () => {
  ...
  const loadProfileImages = async () => {
    try {
      ...
      console.log('Response data:', response.data);  // Přidáno pro debugging
      
      if (response.data.profile_images && Array.isArray(response.data.profile_images)) {
        console.log('Profile images:', response.data.profile_images);  // Přidáno pro debugging
  ...
  const renderProfileItem = ({ item, index }) => {
    console.log('Rendering item:', item);  // Přidáno pro debugging
    const isFromImageDetail = item.from_image_detail === true;
      ...
      <View style={styles.profileItemContainer}>
        <View style={styles.imageContainer}>
          {item.image_url ? (
            ...
            <Text style={styles.noImageText}>Žádný obrázek</Text>
          )}
        </View>
          ...
          {index !== 0 && (
            <>
              <TouchableOpacity 
                ...
                onPress={() => setActiveProfileImage(item.image_url)}
              >
                <Text style={styles.actionButtonText}>Nastavit jako hlavní</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={styles.actionButton} 
                onPress={() => deleteProfileImage(item.image_url)}
              >
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.imageCountText}>Počet obrázků: {profileImages.length}</Text>
