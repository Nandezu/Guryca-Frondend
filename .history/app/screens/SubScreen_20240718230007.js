...
const SubScreen = () => {
  ...
  const fetchSubscriptionDetails = useCallback(async () => {
    try {
      ...
      const localShowBuyButtons = await AsyncStorage.getItem('showBuyButtons');
      setShowBuyButtons(localShowBuyButtons === 'true');
    } catch (error) {
  ...
  useEffect(() => {
    const checkSubscriptionReset = async () => {
      ...
      if (lastSubscription !== userSubscription) {
        setShowBuyButtons(false);
        await AsyncStorage.setItem('showBuyButtons', 'false');
        await AsyncStorage.setItem('lastSubscription', userSubscription);
      }
    };
  ...
  const handleCancelSubscription = async () => {
    try {
      ...
      await AsyncStorage.setItem('showBuyButtons', 'true');
      fetchSubscriptionDetails();
    } catch (error) {
  ...
  const renderButtons = (plan) => {
    ...
    return (
      <View style={[styles.buttonsContainer, { bottom: BUTTONS_BOTTOM_OFFSET }]}>
        ...
        {isCurrentPlan && !plan.isFree && !showBuyButtons && (
        ...
        {(!isCurrentPlan || showBuyButtons) && !plan.isFree && (
          <>
