...
const DressScreen = () => {
  ...
  useEffect(() => {
    if (route.params?.lastViewedIndex !== undefined) {
      setSelectedProfileIndex(route.params.lastViewedIndex);
      flatListRef.current?.scrollToIndex({ index: route.params.lastViewedIndex, animated: false });
    }
  }, [route.params?.lastViewedIndex]);
