...
const SubScreen = () => {
  ...
  const handleCancelSubscription = async () => {
    Alert.alert(
      'Cancel Subscription',
      'Are you sure you want to cancel your subscription? You will lose all the benefits at the end of the current billing period.',
      [
        {
          text: 'No',
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Aktualizujeme stav na free plan a resetujeme limity
              const updatedLimits = {
                virtual_try_ons_remaining: 5,
                profile_images_remaining: 3,
                try_on_results_remaining: 7,
              };
              setUserSubscription('free');
              updateSubscriptionLimits(updatedLimits);

              Alert.alert('Subscription Cancelled', 'Your subscription has been cancelled successfully. You will continue to have access to the benefits until the end of the current billing period.');
            } catch (error) {
      ...
      <View style={[styles.buttonsContainer, { bottom: BUTTONS_BOTTOM_OFFSET }]}>
        {isCurrentPlan ? (
          <>
            ...
            <Text style={[styles.tryOnsText, { marginBottom: 20 }]}>TRY ONS Remaining: {virtualTryOnsRemaining}</Text>
            {!plan.isFree && (
