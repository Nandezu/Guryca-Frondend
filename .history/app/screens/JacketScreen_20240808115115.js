...
const JacketScreen = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const cachedProducts = await AsyncStorage.getItem('cachedJacketProducts');
      ...
      if (!lastShuffleTime || now - parseInt(lastShuffleTime) > 1 * 60 * 1000 || !cachedProducts) {
        ...
        const response = await axios.get(`${BASE_URL}/products/`, {
          ...
          params: { clothing_category: 'jacket' }
        });
        ...
        await AsyncStorage.setItem('cachedJacketProducts', JSON.stringify(selectedProducts));
    ...
    return (
      <View key={product.sku || `product-${index}`} style={styles.productWrapper}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('JacketDetailScreen', { product, userId, initialIndex: selectedProfileIndex })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
