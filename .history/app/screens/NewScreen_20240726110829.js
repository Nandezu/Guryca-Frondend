...
import { View, Image, TouchableOpacity, StyleSheet, FlatList, Alert, Dimensions, Text, ScrollView } from 'react-native';
...
const NewScreen = () => {
  ...
  useEffect(() => {
    if (route.params?.selectedProfileIndex !== undefined) {
      console.log("Updating selectedProfileIndex:", route.params.selectedProfileIndex);
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log("Fetched product SKUs:", fetchedProducts.map(p => p.sku));
      ...
      console.log("Number of shuffled products:", shuffledProducts.length);
      setProducts(shuffledProducts);

      // Vymazat starou cache
      await AsyncStorage.removeItem('lastProductShuffleTime');
      await AsyncStorage.removeItem('shuffledProducts');

      console.log("Products updated and old cache cleared");
  ...
  const handleScroll = (event) => {
    ...
    if (index !== selectedProfileIndex) {
      console.log("Scroll changed selectedProfileIndex to:", index);
  ...
  const renderProfileItem = ({ item, index }) => {
    console.log("Rendering profile item:", index, item.id);
  ...
  const renderProductItem = ({ item: product, index }) => {
    console.log("Rendering product item:", index, product.sku);
    ...
    return (
      <View key={`product-${product.id || product.sku || index}`} style={styles.productWrapper}>
        <TouchableOpacity
          style={styles.productTouchable}
          onPress={() => navigation.navigate('NewDetailScreen', { product, userId, initialIndex: selectedProfileIndex })}
          delayPressIn={100}
          delayPressOut={100}
          delayLongPress={200}
        >
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) => `profile-${item.id || index}`}
        horizontal
        pagingEnabled
