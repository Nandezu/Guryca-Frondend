...
const ManageProfileImage = () => {
  ...
  const selectPhoto = async () => {
    ...
    if (profileImages.length >= maxImagesAllowed) {
      Alert.alert(
        'Limit Reached',
        `Your subscription allows up to ${maxImagesAllowed} profile images. Please delete an existing image before adding a new one.`
      );
      return;
    }
  ...
  const renderProfileItem = ({ item, index }) => {
    ...
    const imageStyle = isFromImageDetail
      ? [styles.profileImage, styles.transformedProfileImage]
      : [styles.profileImage, styles.addNewPhotoImage];
      ...
      <View style={styles.profileItemContainer}>
        <View style={styles.imageContainer}>
          <Image
            source={{ uri: item }}
            style={imageStyle}
            resizeMode={isFromImageDetail ? "cover" : "contain"}
          />
        </View>
...
const styles = StyleSheet.create({
  ...
  addNewPhotoImage: {
    width: '115%',  // Zde můžete upravit šířku nově přidaných fotek
    height: '100%',
  },
  imageActions: {
