...
export default function CodeConfirmScreen({ route, navigation }) {
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://192.168.0.106:8000/user/confirm-email/', {
        email,
        confirmation_code: code
      });

      if (response.status === 200) {
        navigation.navigate('UploadPhoto', { email: email });
      }
    } catch (error) {
  ...
  const handleResendCode = async () => {
    setLoading(true);
    try {
      const response = await axios.post('http://192.168.0.106:8000/user/resend-confirmation/', {
        email
      });
