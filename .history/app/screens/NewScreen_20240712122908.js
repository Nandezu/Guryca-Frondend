...
const NewScreen = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (userId) {
        loadData();
        // Nastavení intervalu pro pravidelné obnovování dat
        const intervalId = setInterval(fetchProducts, 5 * 60 * 1000); // každých 5 minut
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('http://192.168.0.106:8000/products/', {
        params: {
          limit: 200,
          ordering: '-created_at'  // Předpokládá, že backend podporuje řazení podle data vytvoření
        }
      });
