...
import { View, Image, StyleSheet, Linking, StatusBar, AppRegistry, Platform } from 'react-native';
...
import UploadPhoto4 from './app/screens/UploadPhoto4';
import UploadPhoto5 from './app/screens/UploadPhoto5';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    // Initialize RNIap and Purchases
    const initializeIAP = async () => {
      try {
        ...
        console.log('IAP and Purchases initialized successfully');
      } catch (error) {
        console.error('Error initializing IAP or Purchases:', error);
      }
    };
    ...
    return () => {
      // End the connections when the component unmounts
      RNIap.endConnection();
      Purchases.shutdown();
    };
  }, [loaded]);
  ...
  useEffect(() => {
    const updatePurchasesUser = async () => {
  ...
  const handleDeepLink = useCallback((event) => {
    ...
    if (url) {
      ...
      if (parsedUrl.pathname === '/--/reset-password') {
        ...
        if (token && email) {
          navigationRef.current?.navigate('ResetPasswordConfirmScreen', { token, email });
        }
      }
    }
  }, []);
  ...
  if (!loaded) {
    return (
      <View style={styles.splashContainer}>
        <Image source={require('./assets/images/nandezus.png')} style={styles.splashImage} />
      </View>
    );
  }
  ...
  return (
    <SafeAreaProvider>
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#CFD6DE' }}>
          <Stack.Navigator
            initialRouteName="Home"
            screenOptions={{
              headerShown: false,
              animation: 'none',
              contentStyle: { backgroundColor: '#CFD6DE' },
            }}
          >
            ...
            <Stack.Screen 
              name="SubScreen" 
              component={props => <SubScreen {...props} purchases={Purchases} />} 
            />
            <Stack.Screen name="UploadPhoto4" component={UploadPhoto4} />
            <Stack.Screen name="UploadPhoto5" component={UploadPhoto5} />
          </Stack.Navigator>
        </SafeAreaView>
      </NavigationContainer>
    </SafeAreaProvider>
