...
import CodeConfirmScreen from './app/screens/CodeConfirmScreen';
import ForgotPasswordScreen from './app/screens/ForgotPasswordScreen';
import ResetPasswordConfirmScreen from './app/screens/ResetPasswordConfirmScreen';
...
function AppContent() {
  ...
  useEffect(() => {
    const handleDeepLink = (event) => {
      ...
      if (url) {
        ...
        if (parsedUrl.pathname === '/reset-password') {
          ...
          if (token) {
            navigationRef.current?.navigate('ResetPasswordConfirmScreen', { token });
          }
        }
      }
    };
    ...
    return () => {
      // Odstraněno removeEventListener, protože není potřeba v novějších verzích React Native
    };
  }, []);
